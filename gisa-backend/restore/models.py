...
class RestoreCategory(models.Model):
    main_category = models.ForeignKey(MainCategory, on_delete=models.CASCADE, related_name="restore_categories")
...
class RestorePhoto(LifecycleModelMixin, TimeStampedModel):
    restore = models.ForeignKey(Restore, on_delete=models.CASCADE)
    ...
    @classmethod
    def create_photos(cls, restore: Restore, photo_file_list: List[File]) -> List["RestorePhoto"]:
        if not restore.pk:
            raise ValueError("문제를 먼저 저장해주세요.")

        photo_list = []
        for photo_file in photo_file_list:
            photo = cls(restore=restore)
